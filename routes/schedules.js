...
router.get('/:scheduleId', authenticationEnsurer, (req, res, next) => {
    ...
    let storedAllUsers = []; // 全ユーザのオブジェクトを入れる配列 +++
    User.findAll().then((allUsers) => { // +++
        return allUsers; // +++
    }).then((allUsers) => {
        storedAllUsers = allUsers; // +++
        return Schedule.findOne({ // sequelizeオブジェクトを使い、テーブルを結合してユーザー属性を取得
            include: [
                {
                    model: User,
            ...
            console.log('userMap--------------------------------------------------' + Array.from(userMap));
