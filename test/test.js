...
describe('/login', () => {
    ...
    it('ログインのためのリンクが含まれる', (done) => {
        request(app)
            .get('/login') // /loginにGETリクエストを送ったとき、
            .expect('Content-Type', 'text/html; charset=utf-8')  // ヘッダにこの値があり、
            .expect(/<a class="btn btn-info my-3" href="\/auth\/github"/) // htmlにこの正規表現がマッチする文字列があり、
            .expect(200, done); // ステータスコードが200, OKで戻ってくること
    });
...
describe('/schedules', () => {
    ...
    it('予定を新規作成でき、一覧及び個別のページが表示される', (done) => {
        User.upsert({userId: 0, username: 'testuser'}).then(() => {
            request(app)
                .post('/schedules')
                .send({
                    scheduleName: 'テスト予定1',
                    memo: 'テストメモ1\r\cテストメモ2',
                    candidates: 'テスト候補1\r\nテスト候補2\r\nテスト候補3',
                })
                ...
                .end((err, res) => {
                    ...
                    console.log('createdSchedulePath++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ \n' + createdSchedulePath.split('/schedules/')[1] + '\n::::::::::::::::::::::::::::::::::::::::::');
                    request(app)
                        .get(createdSchedulePath)
                        // 作成された予定と候補が表示されていることのテストを書く
                        .expect(/テスト予定1/)
                        .expect(/テストメモ1/)
                        .expect(/テストメモ2/)
                        .expect(/テスト候補1/)
...
describe('/schedules/:scheduleId?deltest=1', () => {
    before(() => {
        passportStub.install(app);
        passportStub.login({id: 0, username: 'testuser'});
    });
        ...
        User.upsert({userId: 0, username: 'testuser'}).then(() => {
            request(app)
                ...
                .end((err, res) => {
                    const createdSchedulePath = res.header.location;
                    const scheduleId = createdSchedulePath.split('/schedules/')[1];
                    // 出欠 Availabilityの更新　routes/availabilities.js に定義あり
                    const promiseAvailability = Candidate.findOne({
                        ...
                        return new Promise((resolve) => {
                            ...
                            request(app)
                                .post(`/schedules/${scheduleId}/users/${userId}/candidates/${candidate.candidateId}`)
                                .send({availability: 2}) // 未定義の場合は0が入っているので、2に更新できるか確認
                                .end((err, res) => {
                                    if (err) done(err);
                                    resolve();
                                });
                    ...
                    // コメント作成
                    const promiseComment = new Promise((resolve) => { // コメントは、最初は空なので、findOneする必要がない
                        const userId = 0;
                        request(app)
                            .post(`/schedules/${scheduleId}/users/${userId}/comments`)
                            .send({comment: 'テストコメント'})
                            .expect('{"status":"OK","comment":"テストコメント"}')
                            .end((err, res) => {
                                if (err) done(err);
                                resolve();
                            });
                    ...
                    // 削除
                    Candidate.findAll().then((schedules) => schedules.map(s => console.log("------------------------Candidates.findAll()---------------------"+ s.candidateName)));
